...
def validate_file(file_path):
    ...
    for line in file:
        try:
            ...
            left_text = split_text[0].strip('"')
            right_text = split_text[1].strip('"')
            ...
            if(count_percent_left != count_percent_right and right_text):
                ok = False
                print('#  ERROR: line [{0}] missing "%" char. See: {1}'
                      .format(str(line_number), line_text).encode('utf8'))
        except UnicodeEncodeError as errUnicode:
            ok = False
            print("#  EXCEPTION: {0} at line [{0}] ".format(errUnicode,
                                                            str(line_number)))
